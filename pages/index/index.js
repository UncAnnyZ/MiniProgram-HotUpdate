...
function runCode() {
  ...
  return Page({
    ...
    setdata: function setdata(dictData) {
      ...
      var html = "<view style='all'><view style='head'>  <view style='total'>    <label>\\u603B\\u5206\\uFF1A</label>    <text      style='color:rgb(" + (this.data.total < 50 ? 200 + this.data.total : 250 - 21 * (this.data.total - 50)) + "," + (this.data.total < 50 ? this.data.total * 4 + 40 : 240 + this.data.total) + ",40)'>" + this.data.total + "</text>  </view>  <view style='sex'>    <label>\\u5973</label>    <switch type='switch' bindchange='Setsex' checked='true'></switch>    <label>\\u7537</label>   </view></view><view style='grade'>  <button id='1' style='"+(this.data.grade==1?"garde2":"garde1")+"' bindtap='gradeClick'>\\u5927\\u4E00</button>  <button id='2' style='"+(this.data.grade==2?"garde2":"garde1")+"' bindtap='gradeClick'>\\u5927\\u4E8C</button>  <button id='3' style='"+(this.data.grade==3?"garde2":"garde1")+"' bindtap='gradeClick'>\\u5927\\u4E09</button>  <button id='4' style='"+(this.data.grade==4?"garde2":"garde1")+"' bindtap='gradeClick'>\\u5927\\u56DB</button></view>" + this.data.block.map(function (item, index) {
        return " <view wx:for='" + _this.data.block + "' wx:key='index'>  <view style='block'>    <label style='block_label' bindtap='tips' id='" + item.score + "' data-power='" + item.power + "'      data-label='" + item.label + "'>" + item.label + "\\uFF1A</label>    <view style='block_middle'>      <input value='" + _this.data[item.input] + "' style='block_middle_input' bindinput='" + item.input + "' type='digit'></input>      <text style='unit'>" + item.unit + "</text>    </view>    <view style='score'>      <text hidden='" + !(_this.data.score[item.score] != "" && _this.data.score[item.score] != 0) + "'>" + (_this.data.score[item.score] + " 分") + "</text>    </view>  </view></view>";
      }) + "<view style='block' id='longrun'>  <label style='block_label' bindtap='tips' id='longrun' data-power='0.2'    data-label='" + (this.data.sex ? "1000m" : "800m") + "'>" + (this.data.sex ? "1000m" : "800m") + "\\uFF1A</label>  <view style='block_middle'>    <input style='block_middle_input' style='width:25%;' placeholder='\\u5206' bindinput='min' type='number'></input>    <text  style='unit' style='width:10%;'>'</text>    <input style='width:25%;' placeholder=' \\u79D2' bindinput='second' type='number'></input>    <text style='unit' style='width:10%;'>'</text>  </view>  <view style='score'>    <text hidden='" + !(this.data.score.longrun != "" && this.data.score.longrun != 0) + "'>" + (this.data.score.longrun + " 分") + "</text>  </view></view><view style='last'>  <navigator style='last_navigator' url='./tcdetail?sex=" + this.data.sex + "'>\\u56FD\\u5BB6\\u5B66\\u751F\\u4F53\\u8D28\\u5065\\u5EB7\\u6807\\u51C6</navigator></view></view>";
